-- Option A: three geographically separated vessels, one active target at a time.
-- Unique keys so the seed can be an idempotent upsert instead of a delete-and-recreate,
-- which would churn the UUIDs the frontend and audits key on.
...
begin
  ...
  values
    ...
    ('9619907', 'MAERSK MC-KINNEY', 'CONTAINER_18000_TEU', 52.80, 3.20, 245, 19.4,
     'UNDERWAY_TRANSIT', 'NORTH_ATLANTIC',
     jsonb_build_array(
       jsonb_build_object('port_name','Rotterdam','locode','NLRTM','lat',51.95,'lon',4.14,
         'eta', to_char(t + interval '14 hours','YYYY-MM-DD"T"HH24:MI:SS"Z"'),
         'customs_ok',true,'has_airport',true,'nearest_airport_iata','AMS','has_reefer_cold_storage',true),
       jsonb_build_object('port_name','Southampton','locode','GBSOU','lat',50.90,'lon',-1.40,
         'eta', to_char(t + interval '2 days','YYYY-MM-DD"T"HH24:MI:SS"Z"'),
       ...
       jsonb_build_object('port_name','New York','locode','USNYC','lat',40.68,'lon',-74.04,
         'eta', to_char(t + interval '9 days','YYYY-MM-DD"T"HH24:MI:SS"Z"'),
  ...
  values
    ...
    ((select id from vessels where imo_number = '9619907'), 'CELL-9011', 'BAY03-R4',
